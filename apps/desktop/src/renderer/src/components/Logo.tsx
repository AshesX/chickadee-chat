...
export function Logo({ size = 24, className, staticLogo = false }: LogoProps): React.JSX.Element {
  ...
  return (
    ...
    >
      ...
      <g mask={`url(#${maskId})`}>
        <circle fill="var(--logo-fill, #f4efe9)" cx={EYE_LEFT.cx} cy={EYE_LEFT.cy} r="34.4" />
        <circle fill="var(--logo-fill, #f4efe9)" cx={EYE_RIGHT.cx} cy={EYE_RIGHT.cy} r="34.4" />
        <path
          fill="var(--logo-fill, #f4efe9)"
