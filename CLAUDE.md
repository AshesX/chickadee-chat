...
- **6D** — push-to-talk (`globalShortcut`), game detection (`ps-list`), noise suppression, per-peer volume, tray, real Settings.
...
- **Still-stubbed (don't mistake for done):** noise-suppression / push-to-talk / volume / real settings are UI-only (→ 6D); game tags need the 6D detector to actually populate. Chat (6B), friends + `userData` persistence (6C) are real. (Friends "invite to room" is intentionally absent — needs a presence channel beyond the in-room model.)
