...
- **6D-ii** — game detection (`ps-list` → `game-state`, already wired), per-peer **volume** sliders, **tray** icon.
...
- **Still-stubbed (don't mistake for done):** the **Volume** control-bar button is a no-op (→ 6D-ii per-peer sliders); game tags need the 6D-ii `ps-list` detector to populate; no tray yet. Chat (6B), friends + `userData` persistence (6C), and push-to-talk / noise-suppression / real Settings (6D-i) are real. (Friends "invite to room" is intentionally absent — needs a presence channel beyond the in-room model.)
