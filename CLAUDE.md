...
- `App.tsx` orchestrates the 3-zone shell and delegates to focused hooks:
  ...
  - `useKeybindSync` (PTT/mute IPC), `useSfxEvents`, `useTraySync`, `useKeyCapture` (+ `lib/accelerator.ts`), `useSpacePresence` (friends list), `useVoiceActivation` (VAD gate), `useMediaDevices`, `useRoomChat`.
