...
- **6D-ii — done (awareness/output):** **game detection** (main scans via Windows `tasklist` against `userData/games.json` → `game-detected` IPC → renderer broadcasts `game-state`; tile short tags + self-status/header full name), **per-peer volume** sliders (`VolumePopover` → `ParticipantTile` `el.volume`, session-only), and a **tray** (runtime canvas-generated placeholder icon → main `Tray`; menu = show / current room / toggle mic / quit). **Phase 6 complete.**
...
- **Phase 6 feature set is fully real.** Intentionally still absent: friends "invite to room" (needs a presence channel beyond the in-room model). Game detection is **Windows-only** (`tasklist`); other OSes report no game. Push-to-talk is a **global toggle** (not hold; `globalShortcut` has no key-up). The tray/sidebar bird is a placeholder 🐦 — swap for the real logo asset when available.
- **Remaining (post-Phase-6):** packaging/distribution to `.exe` (electron-builder), and replacing the placeholder bird emoji / generated tray icon with the real chickadee logo.
