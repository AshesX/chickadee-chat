...
- **6D-i — done (voice controls):** true **push-to-talk** with **hold** or **toggle** mode (persisted; default key **F8**). Hold: mic live only while key physically held; toggle: press to unmute/mute. Powered by `uiohook-napi` (global OS hook, fires when app is out-of-focus/minimized) + Electron's `before-input-event` (fires when window is in focus — Chromium consumes keys before the OS hook fires). Both sources share the `pttIsHeld` flag in main to prevent double-firing. `pttMode: 'hold' | 'toggle'` in `PersistedSettings`. IPC: `ptt-start` / `ptt-stop` (hold) and `ptt-toggle` (toggle). `backgroundThrottling:false` keeps the renderer responsive while minimized. Also: **noise suppression** via Chromium's `noiseSuppression` constraint (`mesh.setNoiseSuppression` / `ensureLocalStream`), and a **real Settings modal** (name + toggles + mode selector + key rebind), all persisted.
